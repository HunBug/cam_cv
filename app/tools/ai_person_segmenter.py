...
class AiPersonSegmenter:
    def __init__(
        self, model_path: Path, labels_path: Path, confidence_threshold: float = 0.5
    ):
        ...
        self._options = ImageSegmenterOptions(
            base_options=self._base_options, output_category_mask=True
        )
    ...
    def segment_background(self, image: np.ndarray) -> np.ndarray:
        if self.segmenter is None:
            raise RuntimeError("Segmenter is not initialized")
        ...
        condition = (
            np.stack((confidence_mask.numpy_view(),) * 3, axis=-1)
